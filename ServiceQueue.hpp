...
#include <queue>
#include "Plane.hpp"
#include "LandingQueue.hpp"
#include "DepartureQueue.hpp"
#include "Random.hpp"
    ...
    void update(int clock)
    {
        ...
        if (!the_queue.empty()) {
            ...
            if ((clock - plane->start_service_time) > plane->service_time) {
                // FIXME: remove plane from the service queue //DONE
                the_queue.pop();
                
                // FIXME: update the enter_departure_time attribute for the plane //DONE
        ...
        if (the_queue.empty()) 
            ...
            if (!landing_queue->the_queue.empty()) {
                ...
                // FIXME: calculate the wait time of the plane in the landing queue //DONE
                int waitTime;
                waitTime = clock - plane->arrival_time;
                
                // FIXME: update total_wait and num_served for the landing queue //DONE
                landing_queue->total_wait += waitTime;
                landing_queue->num_served++;
                
                // FIXME: update the start_service_time attribute for the plane //DONE
                ...
                /* FIXME: compute a random service time for the plane between min_service_time and max_service_time
                 HINT: You can use my_random.next_int(n) to get a random value between 0 and n.  This will help you determine a random number within the range of service times.
                 */ //DONE
                do
                {
                    //should get a random number between 0 and max_service_time
                    plane->service_time = my_random.next_int(max_service_time);
                } while (!(min_service_time < plane->service_time));
                
                // FIXME: add the plane to the service queue //DONE
