...
#ifndef DepartureQueue_hpp
#define DepartureQueue_hpp
        ...
        if (!the_queue.empty()) {
            ...
            if (plane->ready_takeoff_time == -1) { // new plane has arrived at the front
                ...
                if (clock - plane->ready_takeoff_time > departure_time) {
                    ...
                    // FIXME: remove plane from departure queue //DONE
                    the_queue.pop();
                    
                    // FIXME: calculate the wait time //DONE
                    int waitingTime = 0;
                    waitingTime = clock - plane->ready_takeoff_time;
                    
                    
                    // FIXME: update total_wait and num_served //DONE
